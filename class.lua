...
function HTTP:run()
	local addr, port = self.server:getsockname()
	self:log(1, 'listening '..addr..':'..port)
	while true do
		local client	
		if self.block then
...
--self:log(1, 'before unescape, k='..k..' v='..v)							
								k, v = url.unescape(k), url.unescape(v)
--self:log(1, 'after unescape, k='..k..' v='..v)							
								return v, k
							end)
						end
					end
--]]
					...
					if filename then
						...
						assert(send'\r\n')
						if callback then
							--self:log(1, 'wsapi started writing')
							for str in callback do
								assert(send(str))
							end
							--self:log(1, 'wsapi done writing')
						else
